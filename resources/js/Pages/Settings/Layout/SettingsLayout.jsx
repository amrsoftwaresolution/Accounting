...
export default function SettingsLayout({ children, activeTab }) {
    ...
    return (
        ...
        <div className="h-screen bg-[#f4f5f8] flex flex-col overflow-hidden">
            ...
            {/* Top Header */}
            <header className="bg-white border-b border-gray-300 px-6 py-3 flex justify-between items-center shrink-0">
                ...
                <div className="flex items-center gap-4 text-gray-600">
                    <button className="flex items-center gap-1 text-sm hover:underline">
                        <span className="material-icons text-lg">message</span> Give feedback
                    </button>
                    {/* Link to dashboard using the close icon */}
                    <Link
                        href={route('dashboard')}
                        className="material-icons cursor-pointer text-gray-500 hover:text-black transition-colors"
                    >
                        close
                    </Link>
                </div>
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <aside className="w-64 bg-white border-r border-gray-300 shrink-0">
                    <nav className="mt-2">
                        {navItems.map((item) => (
                            <Link
                                key={item.id}
