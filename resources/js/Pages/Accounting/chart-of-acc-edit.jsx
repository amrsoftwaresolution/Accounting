...
export default function ChartOfAccEdit({ chartOfAccount }) {
    ...
    const { data, setData, patch, processing, errors } = useForm({
        ...
        account_type: chartOfAccount.account_type ?? 'asset',
        account_sub_type: chartOfAccount.account_sub_type ?? 'cash-and-cash-equivalents',
        description: chartOfAccount.description ?? '',
        is_active: chartOfAccount.is_active ?? true,
    });

    function submit(e) {
        e.preventDefault();

        patch(route('chart-of-account.update', chartOfAccount.id), {
            onSuccess: () => {
                // optional: show notification by redirect route flash message
            },
        });
    }
                    ...
                    <div className="bg-white p-6 shadow sm:rounded-lg">
                        <form onSubmit={submit} className="space-y-6">
                            ...
                            <div>
                                ...
                                >
                                    {processing ? 'Saving...' : 'Update Chart of Account'}
                                </button>
                            </div>
                        </form>
                    </div>
