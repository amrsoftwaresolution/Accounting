...
export default function SupplierBalance({ reportData, filters, auth }) {
    ...
    const handleFilterChange = (newFilters) => {
        router.get(route('reports.supplier-balance'), { 
    ...
    const handleExportExcel = () => {
        ...
        if (isMonthWise) {
            csvContent += `"Supplier Name","Email","Phone",`;
            monthCols.forEach(m => {
            ...
            csvContent += `\n"Total",,,`;
            monthCols.forEach(m => {
                const mTotalAmt = suppliers.reduce((sum, item) => sum + (item.monthly_balances?.[m] || 0), 0);
        ...
        link.setAttribute("download", `${companyName.replace(/[^a-z0-9]/gi, '_')}_Supplier_Balance_As_Of_${endDate}.csv`);
